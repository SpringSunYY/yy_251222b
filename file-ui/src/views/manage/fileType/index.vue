    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['manage:fileType:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['manage:fileType:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['manage:fileType:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleImport"
          v-hasPermi="['manage:fileType:import']"
        >导入</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['manage:fileType:export']"
        >导出</el-button>
      </el-col>
      ...
      <el-upload ref="upload" :limit="1" accept=".xlsx, .xls" :headers="upload.headers" :action="upload.url + '?updateSupport=' + upload.updateSupport" :disabled="upload.isUploading" :on-progress="handleFileUploadProgress" :on-success="handleFileSuccess" :auto-upload="false" drag>
        ...
        <div class="el-upload__tip text-center" slot="tip">
          <div class="el-upload__tip" slot="tip">
            <el-checkbox v-model="upload.updateSupport" />是否更新已经存在的文件分类数据
          </div>
...
<script>
import { listFileType, getFileType, delFileType, addFileType, updateFileType, importFileType, importTemplateFileType } from "@/api/manage/fileType";
...
export default {
  ...
  data() {
    return {
      ...
      form: {},
      // 导出地址
      exportUrl: 'manage/fileType/export',
      // 文件分类导入参数
      upload: {
        // 是否显示弹出层（文件分类导入）
        open: false,
        ...
        // 上传的地址
        url: process.env.VUE_APP_BASE_API + "/manage/fileType/importData",
        // 下载模板的地址
        templateUrl: 'manage/fileType/importTemplate'
      },
      // 表单校验
      rules: {
        name: [
  ...
  methods: {
    ...
    getList() {
      ...
      listFileType(this.queryParams).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateFileType(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addFileType(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除文件分类编号为"' + ids + '"的数据项？').then(function() {
        return delFileType(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导入按钮操作 */
    handleImport() {
      this.upload.title = "文件分类导入";
      this.upload.open = true;
    },
    /** 下载模板操作 */
    importTemplate() {
      this.download(this.upload.templateUrl, {
      }, `fileType_template_${new Date().getTime()}.xlsx`)
    },
    // 文件上传中处理
