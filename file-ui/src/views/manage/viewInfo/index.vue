<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <!--          size="mini"-->
      <!--          @click="handleAdd"-->
      <!--          v-hasPermi="['manage:viewInfo:add']"-->
      <!--        >新增-->
      <!--        </el-button>-->
      <!--      </el-col>-->
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['manage:viewInfo:edit']"
        >修改
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['manage:viewInfo:remove']"
        >删除
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleImport"
          v-hasPermi="['manage:viewInfo:import']"
        >导入
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['manage:viewInfo:export']"
        >导出
        </el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="viewInfoList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="类型名称" :show-overflow-tooltip="true" align="center" v-if="columns[2].visible"
                       prop="fileTypeName"/>
      <el-table-column label="文件编号" :show-overflow-tooltip="true" align="center" v-if="columns[3].visible"
                       prop="fileId"/>
      <el-table-column label="文件名称" :show-overflow-tooltip="true" align="center" v-if="columns[4].visible"
                       prop="fileName"/>
      <el-table-column label="文件类型" :show-overflow-tooltip="true" align="center" v-if="columns[5].visible"
                       prop="fileType"/>
      <el-table-column label="文件大小" :show-overflow-tooltip="true" align="center" v-if="columns[6].visible"
                       prop="fileSize">
      ...
      <el-table-column label="文件" align="center" v-if="columns[7].visible" prop="fileUrl" width="100">
        <template slot-scope="scope">
      ...
      <el-table-column label="备注" :show-overflow-tooltip="true" align="center" v-if="columns[8].visible"
                       prop="remark"/>
      <el-table-column label="用户" :show-overflow-tooltip="true" align="center" v-if="columns[9].visible"
                       prop="userName"/>
...
import {
  ...
  updateViewInfo,
  importViewInfo,
  importTemplateViewInfo
} from "@/api/manage/viewInfo";
...
export default {
  ...
  data() {
    return {
      ...
      form: {},
      // 导出地址
      exportUrl: 'manage/viewInfo/export',
      // 浏览记录导入参数
      upload: {
        // 是否显示弹出层（浏览记录导入）
        open: false,
        ...
        // 上传的地址
        url: process.env.VUE_APP_BASE_API + "/manage/viewInfo/importData",
        // 下载模板的地址
        templateUrl: 'manage/viewInfo/importTemplate'
      },
      // 表单校验
      rules: {
        fileTypeId: [
  ...
  methods: {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateViewInfo(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addViewInfo(this.form).then(response => {
    ...
    /** 导入按钮操作 */
    handleImport() {
      this.upload.title = "浏览记录导入";
      this.upload.open = true;
    },
    /** 下载模板操作 */
    importTemplate() {
      this.download(this.upload.templateUrl, {}, `viewInfo_template_${new Date().getTime()}.xlsx`)
    },
    // 文件上传中处理
