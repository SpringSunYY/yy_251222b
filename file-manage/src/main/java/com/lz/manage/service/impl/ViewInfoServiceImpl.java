...
public class ViewInfoServiceImpl extends ServiceImpl<ViewInfoMapper, ViewInfo> implements IViewInfoService {
     ...
     * @return 浏览记录
     */
    @DataScope(deptAlias = "tb_view_info", userAlias = "tb_view_info")
    @Override
    public List<ViewInfo> selectViewInfoList(ViewInfo viewInfo) {
        List<ViewInfo> viewInfos = viewInfoMapper.selectViewInfoList(viewInfo);
    ...
    public int insertViewInfo(ViewInfo viewInfo) {
        FileInfo fileInfo = initData(viewInfo);
        fileInfo.setViewCount(fileInfo.getViewCount() + 1);
        fileInfoService.updateFileInfo(fileInfo);
        viewInfo.setUserId(SecurityUtils.getUserId());
        viewInfo.setCreateTime(DateUtils.getNowDate());
        return viewInfoMapper.insertViewInfo(viewInfo);
    }

    private FileInfo initData(ViewInfo viewInfo) {
        //查询文件是否存在
        FileInfo fileInfo = fileInfoService.selectFileInfoById(viewInfo.getFileId());
        if (StringUtils.isNull(fileInfo)) {
            throw new ServiceException("文件不存在");
        }
        viewInfo.setFileId(fileInfo.getId());
        viewInfo.setFileTypeId(fileInfo.getFileTypeId());
        viewInfo.setFileTypeName(fileInfo.getFileTypeName());
        viewInfo.setFileName(fileInfo.getFileName());
        viewInfo.setFileType(fileInfo.getFileType());
        viewInfo.setFileSize(fileInfo.getFileSize());
        viewInfo.setFileUrl(fileInfo.getFileUrl());
        return fileInfo;
    }
